...
class PublicationLikeService {
  ...
  static async likePublication(publicationId: string): Promise<LikeResponse> {
    const response = await api.post(
      `/suarec/publications/${publicationId}/like`,
    );
    return response.data;
  }

  // Quitar like de una publicación
  static async unlikePublication(
    publicationId: string,
  ...
  static async getPublicationLikes(
    publicationId: string,
  ): Promise<LikeResponse[]> {
    const response = await api.get(
      `/suarec/publications/${publicationId}/likes`,
    );
    return response.data;
  }

  // Contar likes de una publicación
  static async getPublicationLikesCount(
    publicationId: string,
  ): Promise<LikeCountResponse> {
    const response = await api.get(
      `/suarec/publications/${publicationId}/likes/count`,
    );
    return response.data;
  }
  ...
  static async checkUserLike(publicationId: string): Promise<UserLikeResponse> {
    const response = await api.get(
      `/suarec/publications/${publicationId}/like/check`,
    );
    return response.data;
  }
