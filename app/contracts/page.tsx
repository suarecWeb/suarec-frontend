                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      {shouldShowPaymentButton(contract) && (
                        <button
                          ...
                          className="px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all duration-200 font-medium shadow-sm flex items-center justify-center gap-2"
                        >
                        ...
                        <div className="px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg">
                          <div className="flex items-center gap-2 text-amber-800">
                            ...
                            <span className="text-sm font-medium">
                              Pago en efectivo: ${contract.totalPrice?.toLocaleString()} al momento del servicio
                            </span>
                          </div>
                        </div>
                      )}
              ...
              <div className="grid gap-6">
                {contracts.asProvider.map((contract) => (
                  <div key={contract.id} className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    {/* Total Price Display for Provider (when contract is accepted) */}
                    {contract.status === ContractStatus.ACCEPTED && contract.totalPrice && (
                      <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4 mb-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <Receipt className="h-5 w-5 text-green-600" />
                            <div>
                              <p className="text-sm font-medium text-green-800">Total del contrato (incluye IVA)</p>
                              <p className="text-2xl font-bold text-green-700">
                                ${contract.totalPrice.toLocaleString()}
                              </p>
                            </div>
