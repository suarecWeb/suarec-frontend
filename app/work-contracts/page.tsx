...
const WorkContractsPageContent = () => {
  ...
  const fetchContracts = useCallback(
          ...
          const role =
            activeTab === "as-client"
              ...
              : activeTab === "as-provider"
                ? "provider"
                : undefined;
          response = await WorkContractService.getUserWorkContracts(
    ...
    [currentUserId, activeTab, userRoles, pagination.limit],
  );
  ...
  }, [currentUserId, activeTab, fetchContracts]);
  ...
  const handleStatusUpdate = async (contractId: string, status: string) => {
    try {
      await WorkContractService.updateWorkContract(contractId, {
  ...
  const handleOpenRatingModal = (contract: WorkContract) => {
    const userToRate =
      contract.client?.id === currentUserId
        ? contract.provider
        : contract.client;
  ...
  const renderContractCard = (contract: WorkContract) => {
    ...
    const canRate =
      ...
      !contract.ratings?.some((r) => r.reviewer?.id === currentUserId);
          ...
          <div className="mb-4 space-y-2">
            {contract.agreed_price && (
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span>
                  ${contract.agreed_price.toLocaleString()}{" "}
            ...
            {contract.estimated_completion && (
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span>
                  Estimado: {formatDate(contract.estimated_completion)}
                </span>
              </div>
            )}
          </div>
            ...
            <div className="flex gap-2">
              ...
              {!isClient && contract.status === "PENDING" && (
                <>
                  ...
                  <button
                    onClick={() =>
                      handleStatusUpdate(contract.id!, "CANCELLED")
                    }
              ...
              {!isClient && contract.status === "ACCEPTED" && (
                <button
                  onClick={() =>
                    handleStatusUpdate(contract.id!, "IN_PROGRESS")
                  }
                  ...
                  <>
                    ...
                    {!loading && !error && filteredContracts.length > 0 && (
                      <div className="mt-6 text-sm text-gray-500 text-center">
                        Mostrando {filteredContracts.length} de{" "}
