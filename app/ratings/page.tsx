...
import RatingService, { ContractReadyForRating } from '@/services/RatingService';
...
export default function RatingsPage() {
  ...
  const [selectedContract, setSelectedContract] = useState<ContractReadyForRating | null>(null);
        ...
        <div className="pt-32 pb-8">
          <div className="max-w-4xl mx-auto px-4">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600 font-eras">Cargando contratos...</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#097EEC] text-white py-8 mt-16">
        <div className="container mx-auto px-4">
          <h1 className="text-2xl md:text-3xl font-eras-bold">Calificaciones pendientes</h1>
          <p className="mt-2 text-blue-100 font-eras text-sm md:text-base">Califica a los usuarios con los que has trabajado para ayudar a la comunidad</p>
        </div>
      </div>
        ...
        {contracts.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
            ...
            <p className="text-gray-600 font-eras">
              Cuando completes un trabajo y se procese el pago, podrás calificar al otro usuario aquí.
            </p>
          </div>
        ) : (
            ...
            {contracts.map((contract) => (
              ...
              >
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <h3 className="font-eras-bold text-gray-900">{contract.otherUser.name}</h3>
                    <p className="text-sm text-gray-500 font-eras">
                      {contract.userRole === 'CLIENT' ? 'Proveedor' : 'Cliente'}
                    </p>
                  </div>
                </div>

                <div className="mb-4">
                  <h4 className="font-eras-medium text-gray-900 mb-1">{contract.contractTitle}</h4>
                  <div className="flex items-center text-sm text-gray-500 font-eras">
                    <Calendar className="h-4 w-4 mr-1" />
                    {new Date(contract.completedAt).toLocaleDateString('es-ES', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}
